...
Load the data, .csv files, from ~data/input folder. Set data to backup. Use backup your data list gets crashed.
```{r}
file_path <- "../data/input/pbp_events_L2_S2022_STRegular_fixed_coords.csv"
...
This code ensures that your data is sorted first by Date, Game.ID, and Time. When these three parameters are identical, it further sorts based on the custom order specified in event_order
```{r}
event_order <- c("SHOT", "puckPossession" ,"interruption", "FACEOFF")
data <- data %>% arrange(Date, Game.ID, Time, match(Event, event_order))
```

Set the Score.State, Away.Score and Home.Score one event later. 
```{r}
...
Count the time between two events.
```{r}
...
Set back to back. Back to back means if a team played yesterday.
```{r}
# Add back to back
teams <- data %>%
  group_by(Date) %>%
...
Change Na from all players to MISSING_PLAYER.
```{r}
...
Add a shift change if one thing changed on the ice. The shift_change_index sums up all the shift_changes. Shift change is 1 if it happens a shift change between the event.
```{r}
# Add shift change 
...
Set 1 if the pp or bp is finished. 
```{r}
...
saveRDS(shifts_grouped_ev, paste0("../data/temp/shifts_grouped_ev_", season_info, ".RData"))
```

If a team shoot while the defense team is changing. Don't punish the player get new on the filed.
```{r optional_chunk, eval = should_run_optional_chunk}
...
Get list of all strikers and defenders with time on ice
```{r}
...
Split the shifts_grouped_ev in offensive home and offensive away.
```{r}
...
Set all player who are goalies to GOALIE.GUY.
```{r}
...
Combine all offensive together.
```{r}
...
Change the offensive col names to the offensive/defensive plus player name.
```{r}
...
Remove the unnessesary values in our dataset. 
```{r}
...
View the result and save it in the output folder with the name below.
```{r}
...
```{r}
# knitr::purl("rapm_calculator.Rmd")
```
